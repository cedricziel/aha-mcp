...
Read a record before you change it. \`aha_search\` returns only a name, type, id and URL - never workflow status, release membership or custom field values - so read the record itself with \`aha_get_feature\`, \`aha_get_epic\`, \`aha_get_idea\`, \`aha_get_initiative\`, \`aha_get_release\`, \`aha_get_goal\` or \`aha_get_key_result\` before writing to it, and report what it currently says if a change looks like it would overwrite someone else's work. Two of these tools replace a whole collection rather than adding to it: \`aha_update_feature_tags\` and \`aha_associate_feature_with_goals\` drop anything left out of the request.
