...
export function buildServerInstructions(subdomain?: string | null): string {
  ...
  return `Aha! (aha.io) is the product management tool used for ${account}, and these tools read and write it directly. Its records are the live, shared source of truth rather than a scratch copy: ideas are incoming customer requests, features and epics are planned work, releases group work by date, and goals and initiatives hold the strategy that work rolls up to, with key results measuring the goals.

Read a record before you change it. \`aha_search\` returns a name, type, reference number and URL, plus vote and endorsement counts on ideas - never workflow status, release membership or custom field values - so read the record itself with \`aha_get_feature\`, \`aha_get_epic\`, \`aha_get_idea\`, \`aha_get_initiative\`, \`aha_get_release\`, \`aha_get_goal\` or \`aha_get_key_result\` before writing to it, and report what it currently says if a change looks like it would overwrite someone else's work. Two of these tools replace a whole collection rather than adding to it: \`aha_update_feature_tags\` and \`aha_associate_feature_with_goals\` drop anything left out of the request.

Records carry two identifiers. \`reference_num\` - PROJA-134, IDEASB-I-6375 - is the one people use and the one the Aha UI shows. \`id\` is an internal number nobody recognises. Name a record by its reference number, and quote it in full: the workspace prefix is part of it, so \`IDEASB-I-6375\` identifies an idea and \`I-6375\` identifies nothing - Aha answers a truncated reference with 404, which reads like a missing record rather than a mistyped one.

Every record also carries an absolute \`url\` to its page in Aha. Use it. When you mention a feature, idea, epic, release, goal or initiative, render a markdown link to that \`url\`, labelled with the reference number and name - the user's next step is usually to open the record. Report search results as a list of such links rather than as raw JSON. \`resource\` is the REST endpoint for a record, not a page a person can read, so never offer it as the link. Key results are the exception: they carry no \`url\`, so name one by its reference number and link its goal instead.`;
}
